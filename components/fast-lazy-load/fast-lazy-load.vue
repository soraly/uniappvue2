<template>
	<view :class="loading || error ? 'fast-lazy-load load' : 'fast-lazy-load'" @click="$emit('click')" :style="{
		...
		<image v-show="!loading && !error" :src="src" :mode="mode" :class="{'show':!loading && !error}" :style="{
			'width':!isNaN(width) ? width + 'rpx' : width,
			'height':!isNaN(height) ? height + 'rpx' : height,
			'border-radius':!isNaN(radius) ? radius + 'rpx' : radius,
