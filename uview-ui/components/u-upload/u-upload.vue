<template>
	<view class="u-upload" v-if="!disabled">
		<view v-if="showUploadList" class="u-list-item u-preview-wrap" v-for="(item, index) in lists" :key="index" :style="{
			...
			<template v-if="item.file.type.indexOf('image') !== -1">
				<image @tap.stop="doPreviewImage(item.url || item.path, index)" class="u-preview-image" v-if="!item.isImage"
					:src="item.url || item.path" :mode="imageMode">
				</image>
			</template>
			<view class="u-preview-image" v-if="item.file.type.indexOf('video') !== -1">
				<video style="width: 100%;height: 100%;" :src="item.url || item.path" controls :show-fullscreen-btn="false"
...
export default {
	...
	props: {
		...
		limitType: {
			...
			default() {
				...
				return ['png', 'jpg', 'jpeg', 'webp', 'gif', 'image', 'mp4', 'mov'];
			}
		},
	...
	methods: {
		...
		selectFile() {
			...
			const { name = '', maxCount, multiple, maxSize, sizeType, lists, camera, compressed, maxDuration, sourceType, limitType } = this;
			let chooseFile = null;
