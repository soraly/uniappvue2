  ...
  <view class="bank-box" :style="{ marginTop: safeHeight }">
    ...
    <view class="bank-row">
      <uni-list class="list-row">
        <uni-list-item class="list-item" :clickable="true" showArrow @click="onClick()">
          <template v-slot:header>
            <view class="slot-box">
              <image class="slot-image" :src="info.logoMobile || '/static/img/user/bank/usdt.png'" mode="widthFix">
              </image>
            </view>
          </template>
          <!-- 自定义 body -->
    ...
    <view class="hint">
      <view class="hint-box">
        ...
        <view class="content">
          ...
          <view class="label">５．
            <view class="text">提现分为：支付宝提现，银行卡提现与微信提现。</view>
          </view>
        </view>
      </view>
    </view>
    ...
    <uni-drawer ref="pay" mode="right" :width="100">
      <List :bankList="bankList" @closePay="closePay" @setBank="setBank" />
    </uni-drawer>
...
import { bankCardList, init, draw } from "../../utils/lottery/mine";
import Playing from '@/utils/common/playing';

export default {
  name: "index",
  components: {
    AlertInjectLayer, UniCol, UniRow, NavBar, List, oneInput, Success, PopItem
  },
  data() {
    return {
  ...
  methods: {
    ...
    allBtn() {
      this.money = this.init.drawAmount
    },
    getInit() {
      init().then(res => {
        if (res.code == 200) {
