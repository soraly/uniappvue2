<template>
  <view class="shishicai-board" :style="{ marginTop: safeHeight, height: windowHeight }">
      ...
      <view class="board">
        ...
        <view class="board-content">
          <view class="board-content-item-left">
            <view class="top">
              <text v-if="this.headInfo.stampType === StampType.InTheEnvelope">{{ `第${headInfo.nextShorNo ? headInfo.nextShorNo : ''}期`
              }}</text>
          ...
          <view class="board-content-item">
            <view class="info">
              ...
              <view class="line3">
                <text v-if="headInfo.stampType === StampType.WaitingForOpening">等待开盘中</text>
                <text v-if="headInfo.stampType === StampType.CutOff">截止</text>
                <text style="color: #FFFF00" v-if="headInfo.stampType === StampType.InTheEnvelope">封单中</text>
              </view>
      ...
      <view class="confirm-box">
        <view class="confirm-div">
          ...
          <view class="btn-box">
            ...
            <button v-if="headInfo.stampType === StampType.InTheEnvelope || headInfo.stampType === StampType.WaitingForOpening" class="confirm-act-close">封单中</button>
...
import templateSelector from "@/config/templateMemSelector/ShishicaiTemplateSelector.js"
      ...
      menuList: [
        {
          name: '最近开奖',
          isSwitch: false,
          url: '/pages/liuhe/ssc/recent_draws',
  ...
  methods: {
    ...
    setRewardsTimeOut(time) {
      ...
      var sendCondRun = function (useSecend) {
        ...
        if (Date.now() + _this.websocketTimeInterval >= _this.headInfo.nextTime) {
          clearInterval(_this.times);
          _this.doCommitUpdateEventBySocket();
          return;
        }
    ...
    getLotteryInfo(e) {
      ...
      if (data.source === 2) {
        if (obj.lotteryId == this.lotteryId) {
          ...
          if (this.headInfo.lotteryNo < obj.lotteryNo && this.menuList[7].isOpen && this.headInfo.stampType !== StampType.InTheEnvelope) {
            this.getBettingList();
            if (this.isShowMiPai) {
    ...
    doUpdateHeaderInfoFromSocket(webscoektMessageData) {
      ...
      console.log("doUpdateHeaderInfoFromSocket", this.headInfo.lotteryNo, content.lotteryNo, this.menuList[7].isOpen, this.headInfo.stampType)
      // console.log("set headInfo", content)
      if (this.headInfo.lotteryNo < content.lotteryNo && this.menuList[7].isOpen && content.stampType !== StampType.InTheEnvelope) {
        this.getBettingList();
        if (this.isShowMiPai) {
          this.$refs.mipai.reset()
        } else {
      ...
      if (content.stampType === StampType.WaitingForOpening && content.msgDetail) {
        this.isCloseBuy = true;
      }
      this.setRewardsTimeOut(this.headInfo.closeTimeStamp)
