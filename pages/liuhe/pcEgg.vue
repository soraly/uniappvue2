          ...
          <view class="board-content-item-left">
            ...
            <view class="ball-list">
              <view class="ball-list-item" 
                ...
                <view class="ball-num-result" :class="ballColor === '1' ? 'ball-red': ( ballColor === '2' ? 'ball-blue' : ( ballColor === '3' ? 'ball-green' : 'ball-gray' ) )">
                  <text class="text">{{ headInfo.stampType === 2 ? totalNumber : "?"  }}</text>
                </view>
            </view>
...
import templateSelector from "@/config/templateMemSelector/PcEggTemplateSelector.js"
...
export default {
  ...
  data() {
    return {
      ...
      navBg: require('./../../static/img/liuhe/bg-nav.png'),
      ...
      menuList: [
        {
          name: '最近开奖',
          isSwitch: false,
          url: '/pages/liuhe/pcEgg/recent_draws',
  ...
  computed: {
    ...
    totalNumber() {
      return this.headInfo.lotteryResult && this.headInfo.resultDetail.split(' ')[0] || ""
    },
    ballColor() {
      return this.headInfo.lotteryResult && this.headInfo.resultDetail.split(' ')[1] || null
    }
  },
  onUnload() {
  ...
  onLoad(option) {
    ...
    if (option.id) {//判断id进入后，获取数据
      console.log(option.id, option, "optionoption")
  ...
  methods: {
    ...
    setOrders(data) {
      ...
      console.log(this.orderList, ball.minNum, "this.orderList")
      // if (ball.minNum === 1) {
