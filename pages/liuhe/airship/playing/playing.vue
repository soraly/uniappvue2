<template>
  <scroll-view scroll-y="true" class="zheng-ma-one-six">
    ...
    <view class="content-list" v-else>
      <view
          class="tab-item"
          v-for="data in playData.children"
          :key="data.id"
      >
        <view class="balls">
          <view v-for="(item, index) in data.children" :key="item.id">
            <view v-if="item.showType == 5">
              ...
              <ball :missOrMatch="missOrMatch" ref="ball" :ball="item" :ball-children="item.children" @changeData="changeChildren"/>
            </view>
