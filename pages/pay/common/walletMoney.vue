      ...
      <view class="input-item">
        <view class="input-row">
          <input class="uni-input" :placeholder-style="placeholderStyle" type="number" @input="moeny" v-model="money"
            :placeholder="'请输入充值USDT（' + info.singleMinLimit + '-' + info.singleMaxLimit + '）'" />
        </view>
      </view>

      <view class="info-box">
        <view class="rate-box">
          <view class="title">兑换比例（USDT/RMB）</view>
          <view class="num">{{ info.exchangeRatio }}</view>
        </view>
        <view class="rate-box">
          <view class="title">到账金额</view>
          <view class="num">{{ getMoney }}</view>
        </view>
      </view>
...
export default {
  name: "bankMoney",
  components: { NavBar, uniDrawer, Wallet },
  props: {
    info: {
      type: Object,
      default: () => {
