<template>
  <view class="log-box" :style="{ marginTop: safeHeight }">
    <NavBar :title="title" :header-style="headerStyle" :bg-color="headerStyle.backgroundColor" :font-color="fontColor"
      ...
      <block slot="right">
        <view class="sx" @click="isShow = !isShow">
          <image src="/static/img/user/accountlog/sx.png"></image>
          筛选
        </view>
      </block>
    </NavBar>
    ...
    <view class="type-box">
      <uni-row class="row-box">
        <uni-col class="row-col" :span="6">
          <view class="title icon-box" @click="isShow = !isShow">
            {{ typeTitle }}
            <text class="icon">
              <uni-icons v-if="isShow" custom-prefix="iconfont" type="uniui-sanjiaoxing_shang-copy" size="14"
                style="font-size: '28rpx';color: #FFFFFF">
              </uni-icons>
              <uni-icons v-else custom-prefix="iconfont" type="uniui-sanjiaoxing_shang" size="14"
                style="font-size: '28rpx';color: #FFFFFF">
              </uni-icons>
            </text>
          </view>
        ...
        <uni-col class="row-col" :span="5">
          <view class="title icon-box">
            账变金额
            <view class="sort-box" @click="setSort('amount')">
              <view class="asc" :class="{ 'act-asc': order == 'asc' && orderBy == 'amount' }">
              </view>
              <view class="desc" :class="{ 'act-desc': order == 'desc' && orderBy == 'amount' }">
              </view>
            </view>
          </view>
        </uni-col>
        <uni-col class="row-col" :span="5">
          <view class="title icon-box">
            余额
            <view class="sort-box" @click="setSort('after_balance')">
              <view class="asc" :class="{ 'act-asc': order == 'asc' && orderBy == 'after_balance' }">
              </view>
              <view class="desc" :class="{ 'act-desc': order == 'desc' && orderBy == 'after_balance' }">
              </view>
            </view>
          </view>
        </uni-col>
          ...
          <view class="container-box" :class="isShow ? 'fadeOutDown' : 'fadeOutUp'">
            <view class="container-row">
              <view class="container-list">
                <view class="container-list-item" :class="{ active: typeTitle == '全部' }" @click="setGroupType()">
                  全部
                </view>
              </view>
            </view>
      ...
      <z-paging :style="'height: calc(var(--vh) - 100rpx - 90rpx - 50rpx - 120rpx);background: #EEEEEE;'" class="paging"
              ...
              <view class="title">
                <view class="bank"
                  :class="item.balanceDirection == 1 ? 'red' : item.balanceDirection == 2 ? 'green' : ''">{{
                    item.balanceDirection == 2 ? '-' : '' }}{{ item.amount }}</view>
              </view>
            </uni-col>
            <uni-col class="row-col" :span="5">
              <view class="title">
...
import { getGroupType, balanceRecord } from "../../../utils/lottery/mine";
import AlertInjectLayer from "@/components/common/alert/AlertInjectLayer.vue"

export default {
  name: "index",
  components: {
    AlertInjectLayer, UniCol, UniRow, NavBar, Common, TimePicker, Change
  },
  data() {
    return {
