<template>
    <view class="mint-tab-container-item home-div" :style="{ marginTop: safeHeight }">
        ...
        <scroll-view :scroll-top="scrollTop" scroll-y="true" class="scroll-Y" show-scrollbar="true"
            ...
            <!--列表日期-->
            <view class="body-image" style="padding-bottom: 130rpx;">
                <view :class="isTop == 1 ? 'van-sticky van-sticky&#45;&#45;fixed' : 'van-sticky'" id="year">
                    <scroll-view scroll-left="100" class="scroll-view_H" scroll-x="true" scroll-with-animation="true"
                        show-scrollbar="false">
                        <view class="scroll-view-item_year" v-for="(item, index) in listYear" :key="index"
                            @click="picture(item)">
                            <view>
                ...
                <view role="feed" class="van-list" id="list">
                    ...
                    <view class="van-col van-col&#45;&#45;12">
                        <view class="image-item" v-for="(item, index) in rightArr" :key="index"
                            @click="pictureDetail(item)">
                            <fast-lazy-load :radius="'8rpx'" :src="item.pictureUrl" :width="'100%'" mode="widthFix">
                            </fast-lazy-load>
                            <view class="image-item-title">{{ item.pictureName }}</view>
                        </view>
            ...
            <view v-if="isLoadMore" class="loading">加载中...</view>

            <view style="clear: both"></view>
        </scroll-view>
        <!--底部彩种-->
        <view class="ft-icon-text">
            <div v-for="(item, index) in lotteryTime" :key="index" @click="cutover(item, index)"
...
import HomePop from "../../liuhe/common/HomePop";
